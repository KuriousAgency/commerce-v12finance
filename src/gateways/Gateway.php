...
class Gateway extends BaseGateway
{
	...
	public function init()
	{
		...
		$this->retailer = [
			'AuthenticationKey' => Craft::parseEnv($this->authKey),
			'RetailerGuid' => Craft::parseEnv($this->retailerGuid),
			'RetailerId' => Craft::parseEnv($this->retailerId),
		];
	}
	...
	public function getSettingsHtml()
	{
		return Craft::$app
			->getView()
			->renderTemplate('v12finance/gatewaySettings', [
				'gateway' => $this,
			]);
	}
	...
	public function populateRequest(
		array &$request,
		BasePaymentForm $paymentForm = null
	) {
	...
	public function getExampleProduct($saleItems = false)
	{
		...
		foreach ($this->getAvailableProducts($saleItems) as $product) {
			if (
				$value == 0 ||
				round($amount * $product->CalculationFactor, 2) < $value
			) {
	...
	public function purchase(
		...
		$applicationResponse = $this->request('SubmitApplication', [
			'json' => $requestData,
		]);
		...
		if (count($applicationResponse->Errors)) {
			$applicationResponse->message =
				$applicationResponse->Errors[0]->Description;
	...
	public function processWebHook(): WebResponse
	{
		...
		if (!$secret || !$stripeSignature) {
			Craft::warning(
				'Webhook not signed or signing secret not set.',
				'stripe'
			);
			$response->data = 'ok';
			return $response;
...
$hash = craft()->request->getParam('SR');
	 ...
	 */
	private function _buildRequestPaymentSource(
		Transaction $transaction,
		Payment $paymentForm,
		array $request
	): Source {
		...
		if ($paymentForm->threeDSecure) {
			...
			$request['redirect'] = [
				'return_url' => UrlHelper::actionUrl(
		...
		throw new PaymentException(
			Craft::t(
				'commerce-stripe',
				'Cannot process the payment at this time'
			)
		);
	}
	/**
	 ...
	 */
	private function _createPaymentResponseFromApiResource(
		$resource
	): PaymentResponse {
	 ...
	 */
	private function _createPaymentResponseFromError(
		\Exception $exception
	): PaymentResponse {
		if ($exception instanceof CardError) {
		...
		} elseif ($exception instanceof Base) {
			...
			$data['code'] =
				$body['error']['code'] ??
				($body['error']['type'] ?? $exception->getStripeCode());
		} else {
			throw $exception;
		}
		return new PaymentResponse($data);
	...
	private function _handleInvoiceCreated(array $data)
	{
		...
		$canBePaid =
			empty($stripeInvoice['paid']) &&
			$stripeInvoice['billing'] === 'charge_automatically';
		if (
			StripePlugin::getInstance()->getSettings()
	...
	private function _handleInvoiceSucceededEvent(array $data)
	{
		...
		if (!$subscription) {
			throw new SubscriptionException(
				'Subscription with the reference “' .
					...
					'” not found when processing webhook ' .
					$data['id']
			);
		}
		$invoice = new Invoice();
		...
		StripePlugin::getInstance()
			->getInvoices()
			->saveInvoice($invoice);
		$lineItems = $stripeInvoice['lines']['data'];
		$currency = Commerce::getInstance()
			->getCurrencies()
			->getCurrencyByIso(strtoupper($invoice->invoiceData['currency']));
		// Find the relevant line item and update subscription end date
		foreach ($lineItems as $lineItem) {
			if (
				!empty($lineItem['subscription']) &&
				$lineItem['subscription'] === $subscriptionReference
			) {
				$payment = $this->_createSubscriptionPayment(
					$invoice->invoiceData,
					$currency
				);
				Commerce::getInstance()
					...
					->receivePayment(
						$subscription,
						$payment,
						DateTimeHelper::toDateTime($lineItem['period']['end'])
					);
				return;
			}
		}
	...
	private function _getStripeCustomer(int $userId): Customer
	{
		try {
			...
			throw new CustomerException(
				'Could not fetch Stripe customer: ' . $exception->getMessage()
			);
		}
	}
	/**
	...
	private function _normalizePaymentToken(string $token = ''): string
	{
		if (StringHelper::substr($token, 0, 4) === 'tok_') {
			try {
				...
				Craft::error(
					'Unable to normalize payment token: ' .
						$token .
						', because ' .
	 ...
	 */
	private function _authorizeOrPurchase(
		Transaction $transaction,
		BasePaymentForm $form,
		bool $capture = true
	): RequestResponseInterface {
		...
		$paymentSource = $this->_buildRequestPaymentSource(
			$transaction,
			$form,
			$requestData
		);
		if (
			$paymentSource instanceof Source &&
			$paymentSource->status === 'pending' &&
			$paymentSource->flow === 'redirect'
		) {
			// This should only happen for 3D secure payments.
			$response = $this->_createPaymentResponseFromApiResource(
				$paymentSource
			);
